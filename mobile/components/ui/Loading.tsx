  ...
  return (
    <View className="flex-1 bg-background">
      <View className="px-6 pb-4 border-b border-black/5 dark:border-white/5 flex-row items-center gap-4" style={{ paddingTop: insets.top + 10 }}>
        ...
        <AppText className="text-text-primary text-xl font-bold">{title}</AppText>
      </View>
      <View className="flex-1 items-center justify-center">
        <ActivityIndicator size="large" color="#6366F1" />
        <AppText className="text-text-tertiary font-bold uppercase tracking-widest mt-6 text-xs">{subtitle}</AppText>
      </View>
    </View>
  );
}